...
fn sidecar_for(path: &str) -> Option<Sidecar> {
    let h = helper()?;
    let lowered = match vikt_rs::lower_file_with(std::path::Path::new(path), &h) {
        Ok(l) => l,
        Err(e) => {
            eprintln!("skipping: {e}");
            return None;
    ...
    let mut side = Sidecar::new(lowered.file.clone(), "vikt-rs/test");
...
fn for_loop_is_loop_carried() {
    ...
    let h = helper().unwrap();
    let lowered =
        vikt_rs::lower_file_with(std::path::Path::new("../../demo/rust/orders.rs"), &h).unwrap();
    let ir = lowered
        .functions
        .iter()
...
fn reports_compile_errors() {
    ...
    let h = helper().unwrap();
    let bad = std::env::temp_dir().join("vikt-rs-bad.rs");
    std::fs::write(&bad, "fn broken( {").unwrap();
    let r = vikt_rs::lower_file_with(&bad, &h);
