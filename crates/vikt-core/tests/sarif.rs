...
use vikt_core::Tier;
use vikt_core::artifact::{Coverage, FunctionRecord, Sidecar, SpanRecord, TierCounts};
use vikt_core::sarif::{SARIF_SCHEMA, SARIF_VERSION, SarifLog};
...
/// mode shape).
fn sample() -> Sidecar {
    let mut side = Sidecar::new("/repo/src/lib.rs", "vikt-core/test");
...
/// one rule per requested tier under the `vikt/<tier>` id scheme.
#[test]
...
fn one_result_per_reported_line() {
    ...
    let first = &results[0];
    assert_eq!(first["ruleId"], "vikt/core");
    ...
    assert_eq!(results[2]["ruleId"], "vikt/boundary");
...
fn core_only_and_foreign_root() {
    ...
    assert_eq!(rules[0]["id"], "vikt/core");
