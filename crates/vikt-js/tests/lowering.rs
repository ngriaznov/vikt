...
use vikt_core::{Denylist, Reason, ScoreWeights, Sidecar, Tier, analyze};
...
fn sidecar_for(source: &str, file: &str) -> Sidecar {
    ...
    let mut side = Sidecar::new(lowered.file.clone(), "vikt-js/test");
...
fn for_of_is_a_real_loop() {
    let lowered = vikt_js::lower_source(SOURCE, "fixture.js").unwrap();
    let ir = lowered
        .functions
        .iter()
    ...
    let looped = sal
        .nodes
        .iter()
        .flat_map(|n| n.reasons.iter().map(vikt_core::Reason::describe))
...
fn lowering_is_deterministic() {
    let a = format!("{:?}", vikt_js::lower_source(SOURCE, "d.js").unwrap());
    let b = format!("{:?}", vikt_js::lower_source(SOURCE, "d.js").unwrap());
    assert_eq!(a, b);
}
...
fn reports_syntax_errors() {
    let r = vikt_js::lower_source("function {{{", "bad.js");
    assert!(r.is_err());
}
...
/// closure-capture gap named in vikt-js's own module docs (see
...
fn closure_uses_include_its_captured_params() {
    let lowered = vikt_js::lower_source(MEMOIZE_SOURCE, "memoize.js").unwrap();
    let memoize = lowered
        .functions
        .iter()
...
fn labelled_continue_is_a_real_back_edge() {
    let lowered = vikt_js::lower_source(LABELLED_CONTINUE_SOURCE, "labelled.js").unwrap();
    let f = lowered
        .functions
        .iter()
...
fn closures_and_labelled_continue_are_deterministic() {
    let a = format!(
        "{:?}",
        vikt_js::lower_source(MEMOIZE_SOURCE, "d.js").unwrap()
    );
    let b = format!(
        "{:?}",
        vikt_js::lower_source(MEMOIZE_SOURCE, "d.js").unwrap()
    ...
    let c = format!(
        "{:?}",
        vikt_js::lower_source(LABELLED_CONTINUE_SOURCE, "d2.js").unwrap()
    );
    let d = format!(
        "{:?}",
        vikt_js::lower_source(LABELLED_CONTINUE_SOURCE, "d2.js").unwrap()
