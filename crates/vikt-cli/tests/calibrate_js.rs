...
fn mixed_language_trees_pick_the_larger_and_say_so() {
    ...
    let dir = std::env::temp_dir().join(format!("vikt-calibrate-mixed-{}", std::process::id()));
...
fn node_modules_is_present_but_never_scored_or_mutated() {
    ...
    let dir = std::env::temp_dir().join(format!("vikt-calibrate-nodemods-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
...
fn typescript_sources_print_the_type_check_caveat() {
    ...
    let dir = std::env::temp_dir().join(format!("vikt-calibrate-ts-{}", std::process::id()));
    let _ = std::fs::remove_dir_all(&dir);
    std::fs::create_dir_all(&dir).expect("temp dir is writable");
    std::fs::write(
        dir.join("clamp.ts"),
