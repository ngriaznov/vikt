...
use vikt_core::{Denylist, ScoreWeights, Scorer, Sidecar, analyze_with_scorer};
...
impl Stats {
    fn record(&mut self, ir: &FunctionIr) {
        ...
        // VIKT_SCORER=current measures the incumbent alone; default is the
        ...
        let scorer = match std::env::var("VIKT_SCORER").as_deref() {
            Ok("current") => Scorer::Current,
            _ => Scorer::Panel(vikt_core::PanelProfile::Instruction),
        };
    ...
    match mode.as_str() {
        "jvm" => {
            for path in walk(&root, "class").into_iter().take(limit) {
                ...
                let lowered = std::panic::catch_unwind(|| vikt_jvm::lower_class(&bytes));
                match lowered {
                    Err(_) => {
