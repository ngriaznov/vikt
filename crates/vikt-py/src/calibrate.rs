//! Mutant generation for `vikt calibrate`: `.py` source in, mutated
//! sources out.
//!
...
// Re-exported so existing callers of `vikt_py::calibrate::{Mutant,
// MutantSet}` keep working now that the wire type moved to vikt-core —
// see that module's docs for why it moved.
pub use vikt_core::mutant::{Mutant, MutantSet};
...
pub fn mutants_for(
    ...
    script.push(format!("vikt-calibrate-{}-{seq}.py", std::process::id()));
    {
