...
This directory holds three narrative reports: this file (the standing weight-refit decision, regenerated by this script on every run), `ast-fallback-comparison.md` (the tree-sitter fallback measured against each language's primary lowering), and `markupsafe-anomaly.md` (a diagnosed case where the panel lost to the positional null). Every other file here -- a `*.jsonl` next to its `*.meta.json` -- is calibration-run data feeding the numbers above, not narrative.
